...
function drawGuide(ctx) {
  var left =  leftRingCenter();
  var right = rightRingCenter();
  var middle = middlePoint();
  var segmentAngle = 2 * Math.PI / verticalSegments.length;
  var middleHeight = totalRadius() * Math.tan(segmentAngle + 0.08);
  ctx.beginPath();
  ctx.moveTo(left.x, left.y);
  ctx.lineTo(middle.x, middle.y + middleHeight);
